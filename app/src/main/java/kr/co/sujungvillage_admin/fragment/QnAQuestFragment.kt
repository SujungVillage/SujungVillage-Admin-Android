...
class QnAQuestFragment : Fragment() {
    ...
    fun loadQuestionData(token: String, recycleQuestion: RecyclerView) {
        // 내 질문 리스트 조회 API 연결
        RetrofitBuilder.qnaApi.questionGet(token).enqueue(object: Callback<List<QuestionGetResultDTO>> {
            override fun onResponse(call: Call<List<QuestionGetResultDTO>>, response: Response<List<QuestionGetResultDTO>>) {
                ...
                for (info in response.body()!!) {
                    questionList.add(QuestionGetResultDTO(info.id, info.title, info.date, info.isAnswered))
                }
