...
class FAQWriteActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnUpload.setOnClickListener{
            ...
            if(question.isEmpty()){
                if(answer.isEmpty()){
                    Toast.makeText(this,"제목과 내용을 입력하세요. ${question}, ${answer}", Toast.LENGTH_SHORT).show()
                }
                else{
                    Toast.makeText(this,"제목을 입력하세요. ${question}, ${answer}", Toast.LENGTH_SHORT).show()
                }
            }
            else {
                if (answer.isEmpty()) {
                    ...
                    RetrofitBuilder.qnaApi.faqWrite(token,faqWriteInfo).enqueue(object: Callback<FaqWriteResultDTO> {
                        override fun onResponse(call: Call<FaqWriteResultDTO>, response: Response<FaqWriteResultDTO>) {
